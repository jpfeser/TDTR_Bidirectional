...
Tools for analyzing time-domain thermoreflectance experiments (“Bi” referes to Bidirectional heat flow with concentric pump-probe and radially anisotropic stack of materials and interfaces)

I.  In the "Main_Calls" folder, there are two files "BiTDTR_Main.m" and "BiTDTR_Fit.m" that can accomplish nearly everything by calling the other subroutines 
...
The "BiTDTR_Main.m" can by default:
...
To calculate sensitivity plots, based on the parameters you typed in during step 1, set "senseplot=1;"  To skip this step, set "senseplot=0;"  I recommend always looking at the sensivity plots.
	...
	In order to calculate the errorbars, the program needs to know what the "percent error" is for all of the parameters you typed into step 1.  Type them in in the sections labeled "ERRORBAR OPTIONS."  For example, if the heat capacity of layer 3 has a 5% errorbar, then type "C(3)=0.05"  You can skip the calculation for layers which you know won't introduce large errors in the solution using the "_consider" variables.  For example, to not consider layer 3's (underside layer) thermal conductivity, type "lambda_down_consider(3)=0" otherwise, you should consider every layer except the layers that you are solving for (because changing your initial guess for that layer should not affect the solution).


	Step 5: (Optional:  Only used if you are fitting TDTR data or calculating errorbars):  You need to alter the "BiTDTR_Main.m" program and the "BiTDTR_Fit.m" program so that the program knows which variables you are fitting for (it can do simultanous/multivariable fits!).  Here's how you do it (two steps):

A) In the "BiTDTR_Fit.m" program (the purpose of this program is to evaluate the sum-of-the-squares of the residuals, comparing experiment and simulation), you need to define which variables to "fit."  The vector of fitting variables is called "X" in th program.  So for example, to fit for the thermal conductivity of the 3rd layer and the thermal conductivity of the 2nd layer, you would type:

Lambda_down(3) = X(1); %this might represent substrate thermal conductivity, of the underside layer
Lambda_down(2) = X(2); %this might represent thermal interface conductance of the underside layer

B) In the "PROGRAM OPTIONS" section of the "BiTDTR_Main.m" program, you will need to alter a variable called  "X_Fit_Variables" that is used to set the initial guesses for the "fitting parameters." The easiest way to do this, is to type something like (continuing the example from above):

X_Fit_Variables = [lambda_down(3) lambda_down(2)];

%OR

X_Fit_Variables(1) = lambda_down(3); %this represents initial guess for substrate thermal conductivity
X_Fit_Variables(2) = lambda_down(2); %this represents initial guess for thermal interface conductance
