...
function [deltaT,ratio]=BiTDTR_REFL(tdelay,TCR,tau_rep,f,lambda_down,C_down,h_down,eta_down,lambda_up,C_up,h_up,eta_up,r_pump,r_probe,A_pump)
...
dT1_down=zeros(1,length(mvect))';
dT2_down=zeros(1,length(mvect))';
dT1_up=zeros(1,length(mvect))';
dT2_up=zeros(1,length(mvect))';
dT1=zeros(1,length(mvect))';
dT2=zeros(1,length(mvect))';

kmax=1/sqrt(r_pump^2+r_probe^2)*1.5;

    dT1_down=rombint_multi(@(kvect) TDTR_TEMP(kvect,mvect/tau_rep+f,lambda_down,C_down,h_down,eta_down,r_pump,r_probe,A_pump),0,kmax,length(mvect));
    dT2_down=rombint_multi(@(kvect) TDTR_TEMP(kvect,mvect/tau_rep-f,lambda_down,C_down,h_down,eta_down,r_pump,r_probe,A_pump),0,kmax,length(mvect));
    
    dT1_up=rombint_multi(@(kvect) TDTR_TEMP(kvect,mvect/tau_rep+f,lambda_up,C_up,h_up,eta_up,r_pump,r_probe,A_pump),0,kmax,length(mvect));
    dT2_up=rombint_multi(@(kvect) TDTR_TEMP(kvect,mvect/tau_rep-f,lambda_up,C_up,h_up,eta_up,r_pump,r_probe,A_pump),0,kmax,length(mvect));
    
    dT1 = (dT1_down.*dT1_up)./(dT1_down + dT1_up);
    dT2 = (dT2_down.*dT2_up)./(dT2_down + dT2_up);
